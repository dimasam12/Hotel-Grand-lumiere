...
// Tambah filter hotel_id dari env
$stmt = $conn->prepare('SELECT id, username, email, password FROM users WHERE (username = ? OR email = ?) AND role = "user" AND hotel_id = ? LIMIT 1');
$stmt->bind_param('ssi', $identifier, $identifier, $hotel_id);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    $_SESSION['login_attempts']++;
    echo json_encode(['status' => 'error', 'message' => 'Akun tidak ditemukan di aplikasi ini.']);
    $stmt->close();
    $conn->close();
    exit;
}
...
$_SESSION['hotel_id']   = $user['hotel_id']; // simpan hotel_id di session
